...
static core::OneEuro g_fx, g_fy;   // smoothing of the projected screen point
...
static uintptr_t imgui_cb(uint32_t not_charsel_or_loading, uint32_t /*hide*/) {
    if (!not_charsel_or_loading) { g_fx.reset(); g_fy.reset(); return 0; }

    core::AvatarState avatar; core::CameraState cam;
    if (!g_reader.sample(avatar, cam)) { g_fx.reset(); g_fy.reset(); return 0; }

    ImVec2 sz = ImGui::GetIO().DisplaySize;
    core::ScreenPoint feet = core::world_to_screen(avatar.position, cam, sz.x, sz.y);
    if (feet.behind || !feet.on_screen) { g_fx.reset(); g_fy.reset(); return 0; }

    float dt = ImGui::GetIO().DeltaTime;
    feet.x = g_fx.filter(feet.x, dt);
    feet.y = g_fy.filter(feet.y, dt);
